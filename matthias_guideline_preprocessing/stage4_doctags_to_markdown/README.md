...
- Convertion du doctag en markdown# Étape 4 – Finalisation de la pipeline : Conversion Doctags → Markdown
...
2. **Parsing et conversion**
   ...
   - Conversion automatique de chaque balise structurée en syntaxe Markdown :
     - Titres (`section_header_level_X`) → `##`, `###`, etc.
     - Paragraphes (`text`) → texte simple
     - Listes (`unordered_list`, `list_item`) → listes Markdown
     - Tableaux (`otsl`) → tableaux Markdown
     - Images (`picture` ou description d’image) → bloc figure ou citation
     - Liens enrichis → syntaxe `[texte](url)`
     - Séparateurs de page → `---`
...
- **convert_doctags_to_markdown.py**  
