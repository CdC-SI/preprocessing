"""image-description stage, description of doctags images via VLM.

Conversion of `description_image/description_image_context.py`.

ONLY AUTHORIZED EXCEPTION** to invariant no. 1: the script was
not “sync”; it was a thread pool (`queue.Queue` + N `threading.Thread` + `Lock`).
The rewrite is LIMITED TO DISPATCH:

| Before (threads)                  | After (async)                         |
| --------------------------------- | ------------------------------------- |
| task_q: queue.Queue + ImageTask   | list of coroutines for gather         |
| N × threading.Thread(_vlm_worker) | asyncio.Semaphore(n) around the call  |
| results_lock around writes        | no more lock (one task at a time)     |
| None sentinels to stop            | gather finishes on its own            |
| vision_completion (sync)          | await vlm.vision_completion(...)      |
| CLI flag --workers                | max_concurrency attribute (default 1) |

The description order ALWAYS follows the image order (results indexed by
`task.index`, consumed via `sorted(results)`), not the order in which
responses arrive.

All extraction, context, and writing functions are MOVED as-is.
"""
...
# Async dispatch, REPLACES the thread pool (_vlm_worker + queue + lock).
# Structure copied from url_tuning (Semaphore + gather). Trap P5.
async def _describe_task(
    task: ImageTask,
    semaphore: asyncio.Semaphore,
    vlm: AsyncVlmClient,
) -> tuple[int, VLMResult]:
    """Async equivalent of the body of _vlm_worker: one VLM call per image,
    result indexed by task.index, final order follows the images."""
...
class ImageDescriptionStep(PipelineStep):
    ...
    description = "Images description with VLM"
    requires_vlm = True
